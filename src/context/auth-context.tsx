...
export const AUTHOR_PROFILE_SETTINGS_PATH = '/author/profile/settings'; // Defined once
...
const convertFirestoreTimestampToISO = (timestamp: any): string | null => {
  ...
  return String(timestamp); // Fallback, though ideally should not reach here for valid timestamps
};
...
const ensureFirestoreUserProfile = async (
  ...
  try {
    ...
    const finalSnap = await getDoc(userDocRef); // Re-fetch to get server timestamps and merged data
    if (finalSnap.exists()) {
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (!isMounted || !firebaseAuth || !firestoreDb) {
      // If Firebase services aren't ready (e.g., during initial page load or if init failed),
      // ensure loading is false to prevent infinite spinner, but don't proceed with auth logic.
      // The error message for failed Firebase init is handled by the return statement at the end of AuthProvider.
      setLoading(false);
      return;
    }
    ...
    if (justCompletedProfile) {
      ...
      return; // Let the redirect from updateUserProfile take precedence for one cycle
    }
    ...
    const unsubscribe = onAuthStateChanged(firebaseAuth, async (firebaseUser: FirebaseUser | null) => {
      ...
      if (firebaseUser) {
        ...
        if (appUser) {
          ...
          if (!isProfileComplete && pathname !== AUTHOR_PROFILE_SETTINGS_PATH && !pathname.startsWith(AUTHOR_PROFILE_SETTINGS_PATH + '?')) {
            ...
            if (typeof window !== 'undefined') {
              ...
              if (redirectAfterLoginPath === AUTHOR_PROFILE_SETTINGS_PATH || redirectAfterLoginPath?.startsWith(AUTHOR_PROFILE_SETTINGS_PATH + '?')) {
                localStorage.removeItem('redirectAfterLogin'); // Avoid redirecting back to settings
            ...
            if (redirectAfterLoginPath === '/user/profile/settings' || redirectAfterLoginPath === '/profile/settings') {
                correctedRedirectPath = AUTHOR_PROFILE_SETTINGS_PATH; // Correct old paths
            }
            if (correctedRedirectPath === AUTHOR_PROFILE_SETTINGS_PATH && isProfileComplete) {
                if (typeof window !== 'undefined') localStorage.removeItem('redirectAfterLogin');
                const targetDashboard = appUser.isAdmin ? ADMIN_DASHBOARD_PATH : (appUser.role === "Reviewer" ? REVIEWER_DASHBOARD_PATH : AUTHOR_DASHBOARD_PATH);
                router.push(targetDashboard);
            } else if (correctedRedirectPath === AUTHOR_PROFILE_SETTINGS_PATH && !isProfileComplete) {
                 if (typeof window !== 'undefined') localStorage.setItem('completingProfile', 'true');
                 router.push(`${AUTHOR_PROFILE_SETTINGS_PATH}?complete=true`); // Ensure query param is there
            } else {
                if (typeof window !== 'undefined') localStorage.removeItem('redirectAfterLogin');
          ...
          if (firebaseAuth) await signOut(firebaseAuth); // Log out from Firebase Auth
          setUser(null);
        ...
        if (isProtectedGenericPath && !isAuthPage && typeof window !== "undefined" && isMounted && !showLoginModal) {
    ...
    return () => unsubscribe();
  }, [pathname, router, searchParamsFromHook, isMounted, showLoginModal, user, justCompletedProfile]); // Added user and justCompletedProfile
    ...
    try {
      if (!identifier.includes('@')) {
        ...
        if (!querySnapshot.empty) {
          ...
          else throw new Error("Invalid email/username or password.");
        } else throw new Error("Invalid email/username or password.");
      }
  ...
  const signup = async (data: SignupFormValues) => {
    ...
    setLoading(true);
    try {
      // Username and phone number uniqueness checks are now primarily in updateUserProfile
      ...
      // pass signup data for initial profile creation.
      ...
      toast({ title: "Signup Successful", description: "Your account has been created. Please complete your profile if prompted." });
    } catch (error: any) {
  ...
  const handleSocialLoginError = (error: any, providerName: string) => {
    ...
    if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/cancelled-popup-request') {
      toastMessage = `The ${providerName} sign-in popup was closed before completing. Please ensure popups are allowed and try again. If the issue persists, your browser might be blocking them restrictively.`;
  ...
  const processSocialLogin = async (providerName: 'google' | 'github') => {
    ...
    try {
      ...
      setShowLoginModal(false);
      toast({ title: `${providerName.charAt(0).toUpperCase() + providerName.slice(1)} Sign-In Successful`, description: "Welcome!" });
      // onAuthStateChanged will handle profile creation/fetching and redirection
    } catch (error) {
      handleSocialLoginError(error, providerName);
    } finally {
      setLoading(false); // Ensure loading is false even if onAuthStateChanged takes over
      setActiveSocialLoginProvider(null);
    }
  };
  ...
  const sendPasswordResetEmail = async (emailForReset: string) => {
    ...
    try {
      ...
      throw error; // Re-throw for form to catch and display specific message
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateUserProfile = async (updatedData: Partial<Pick<User, 'displayName' | 'username' | 'phoneNumber' | 'institution' | 'researcherId' | 'role'>>) => {
    ...
    try {
      ...
      const updatedUserFromDb = await ensureFirestoreUserProfile(currentFirebaseUser); // Re-fetch to get canonical data
      
      if (updatedUserFromDb) {
        setUser(updatedUserFromDb); // Optimistic update, then confirmed by re-fetch
        ...
        const completingProfileFlag = typeof window !== 'undefined' ? localStorage.getItem('completingProfile') === 'true' : false;

        if (isProfileNowComplete && completingProfileFlag) {
          ...
          if (typeof window !== 'undefined') {
            ...
            const target = (storedRedirectPath && storedRedirectPath !== AUTHOR_PROFILE_SETTINGS_PATH && !storedRedirectPath.startsWith(AUTHOR_PROFILE_SETTINGS_PATH + '?'))
              ? storedRedirectPath
              : (updatedUserFromDb.isAdmin ? ADMIN_DASHBOARD_PATH : (updatedUserFromDb.role === "Reviewer" ? REVIEWER_DASHBOARD_PATH : AUTHOR_DASHBOARD_PATH));
            router.push(target);
          }
        }
  ...
  if (!isMounted || (loading && !user && !firebaseAuth?.currentUser && !pathname.startsWith(HOME_PATH) && pathname !== LOGIN_PATH && pathname !== SIGNUP_PATH) ) {
    return (
