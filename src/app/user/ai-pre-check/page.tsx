...
import { Sparkles, Info, UploadCloud, Download, AlertTriangle } from 'lucide-react'; // Added AlertTriangle
...
const preCheckSchema = z.object({
  ...
  file: z.any() 
    .optional()
    .refine(files => {
      if (typeof window === 'undefined' || !files || !(files instanceof FileList) || files.length === 0) return true; // Allow empty if paperText provided
...
function AiPreCheckContent() {
  ...
  const handleRunAIChecks = async (data: PreCheckFormValues) => {
    ...
    if (fileToUpload) {
        plagiarismInputUrl = `file://${fileToUpload.name}`; 
...
export default function UserAiPreCheckPage() {
  return (
    <ProtectedRoute>
      <AiPreCheckContent />
    </ProtectedRoute>
