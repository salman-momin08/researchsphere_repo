...
import { Input } from '@/components/ui/input'; // Keep for file input styling reference if needed
...
function AiPreCheckContent() {
  ...
  const { control, handleSubmit, watch, setValue, formState: { errors, isValid, isDirty } } = useForm<AiPreCheckFormValues>({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      setValue('file', undefined as any, { shouldValidate: true, shouldDirty: true }); // Use 'any' to satisfy TS if FileList can't be undefined
  ...
  const onSubmit = async (data: AiPreCheckFormValues) => {
    ...
    try {
      // Acceptance probability still primarily based on title and abstract
  ...
  const handleDownloadFeedback = () => {
    ...
    let feedbackContent = `AI Pre-Check Feedback for Paper: ${watch('title') || 'Untitled'}\n\n`;
    if (plagiarismResult) {
    ...
    a.href = url;
    a.download = `${(watch('title') || 'untitled_paper').replace(/\s+/g, '_')}_AI_Feedback.txt`;
      ...
      <Card className="w-full max-w-2xl mx-auto shadow-xl">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-sm sm:text-base">
            Get AI-powered insights on plagiarism and acceptance probability. All fields are mandatory for this pre-check.
          </CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit(onSubmit)}>
          <CardContent className="space-y-4 sm:space-y-6">
            ...
            <div>
              <Label htmlFor="title" className={cn(errors.title && "text-destructive")}>Paper Title *</Label>
              <Controller
                name="title"
                control={control}
                render={({ field }) => <Input {...field} id="title" placeholder="Enter your paper title" className="mt-1" disabled={isLoading} />}
              />
              {errors.title && <p className="text-sm text-destructive mt-1">{errors.title.message}</p>}
            </div>
            <div>
              <Label htmlFor="abstract" className={cn(errors.abstract && "text-destructive")}>Abstract *</Label>
              <Controller
                name="abstract"
                control={control}
                render={({ field }) => <Textarea {...field} id="abstract" placeholder="Paste your paper abstract here" rows={8} className="mt-1" disabled={isLoading} />}
              />
              {errors.abstract && <p className="text-sm text-destructive mt-1">{errors.abstract.message}</p>}
            </div>
            <div>
              <Label htmlFor="file-upload-ai" className={cn(errors.file && "text-destructive")}>Upload Paper (PDF, DOCX, or TXT, max 10MB) *</Label>
            ...
            <Alert variant="default">
              ...
              <AlertDescription className="text-xs sm:text-sm">
                The AI analyzes your title and abstract. If a `.txt` file is uploaded, its content is also included. For PDF/DOCX files, the AI considers the file's name and type for contextual understanding in its simulated plagiarism check. Full file content parsing for PDF/DOCX is not performed in this client-side pre-check. Acceptance probability is based on title and abstract.
              </AlertDescription>
            </Alert>
          </CardContent>
